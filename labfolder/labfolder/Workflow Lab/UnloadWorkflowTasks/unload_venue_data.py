...
# MAGIC This notebook is designed to connect to the Redshift cluster and extract Venue table data in parquet format and store the data in an S3 bucket.
# MAGIC
...
table_name = "venue_parquet_worklow"
...
try:
    ...
    unload_sql = f"""
        ...
        TO 's3://redshiftprocessing/{safe_user}/venue/parquet/'
        IAM_ROLE 'arn:aws:iam::633690268896:role/service-role/AmazonRedshift-CommandsAccessRole-20250505T135309'
        ALLOWOVERWRITE 
        FORMAT AS PARQUET;
    """
...
prefix = safe_user +'/venue/parquet'  # Make sure to end with '/' for folder
...
schema = StructType([
    ...
    StructField("venuecapacity", IntegerType(), True),
...
# Save it as a Delta table if it doesn't exist
   if not spark.catalog.tableExists(full_table_name_parquet):
    ...
    print(f"Error creating venue_parquet_worflow: {e}")
...
FROM 's3://redshiftprocessing/{safe_user}/venue/parquet/'
FILEFORMAT = PARQUET
...
# Construct the SQL query using f-string
sql_query = f"""
select venueid, venuename from  {full_table_name}
"""
