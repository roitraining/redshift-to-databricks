...
# MAGIC This notebook is designed to connect to the Redshift cluster and extract event table data in parquet format and store the data in an S3 bucket.
# MAGIC
...
#dbutils.widgets.text("table_name", "event_delta_51")
...
table_name = "event_parquet_worklow"
full_table_name = f"{uc_schema_name}.{table_name}"
print(full_table_name)
...
The {safe_user} variable was derived from the username of the current user. It is used to create a unique directory in the S3 bucket for this user's data.

You can ignore the message "SLF4J: Failed to load class "org.slf4j.impl.StaticLoggerBinder"" and "SLF4J: Defaulting to no-operation (NOP) logger implementation" if you see them. They are related to the logging library used by the JDBC driver and can be safely ignored.
...
try:
    ...
    unload_sql = f"""
        ...
        TO 's3://redshiftprocessing/{safe_user}/event/parquet/'
        IAM_ROLE 'arn:aws:iam::633690268896:role/service-role/AmazonRedshift-CommandsAccessRole-20250505T135309'
        ALLOWOVERWRITE 
        FORMAT AS PARQUET;
    """
...
prefix = safe_user +'/event/parquet'  # Make sure to end with '/' for folder
...
from pyspark.sql.types import StructType, StructField, IntegerType, StringType, ShortType, VarcharType, TimestampType

# Define the schema
schema = StructType([
    StructField("eventid", IntegerType(), True),
    StructField("venueid", ShortType(), True),
    StructField("catid", ShortType(), True),
...
# Save it as a Delta table if it doesn't exist
   if not spark.catalog.tableExists(full_table_name_parquet):
    ...
    print(f"Error creating event_parquet_worflow: {e}")
...
FROM 's3://redshiftprocessing/{safe_user}/event/parquet/'
FILEFORMAT = PARQUET
...
# Construct the SQL query using f-string
sql_query = f"""
select eventid, venueid, eventname from  {full_table_name}
"""
