...
# MAGIC This notebook is designed to connect to the Redshift cluster and extract sales table data in parquet format and store the data in an S3 bucket.
# MAGIC
...
#dbutils.widgets.text("table_name", "sales_delta_51")
...
table_name = "sales_parquet_worklow"
full_table_name = f"{uc_schema_name}.{table_name}"
print(full_table_name)
...
# MAGIC %md
# MAGIC Another option to determine the correct datatype for the RedShift numeric field, a table was created using just the parquet file and the data types were infered. 

# COMMAND ----------

table_name = "sales_parquet_worklow_noschema"
full_table_name_parquet = f"{uc_schema_name}.{table_name}"
print(full_table_name_parquet)
from pyspark.sql.types import StructType, StructField, IntegerType, StringType, ShortType, DecimalType, TimestampType

df = spark.read.parquet(f"s3://redshiftprocessing/{safe_user}/sales/parquet/")
try:
   if not spark._jsparkSession.catalog().tableExists(full_table_name_parquet):
       df.write.format("delta").saveAsTable(full_table_name_parquet)
except Exception as e:
    print(f"Error creating sales_parquet_worflow_noschema: {e}")

try:
   spark.table(full_table_name_parquet).printSchema()
except Exception as e:
    print(f"Error creating sales_parquet_worflow_noschema: {e}")
...
try:
   spark.sql(sql_query)
   print(f"Data copied into {full_table_name} successfully.")
except Exception as e:
    print(f"Error copying data into {full_table_name}: {e}")
