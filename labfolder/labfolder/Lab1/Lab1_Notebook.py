...
try:
    ...
    unload_sql = f"""
        ...
        TO 's3://redshiftprocessing/{safe_user}/sales/parquet/'
        IAM_ROLE 'arn:aws:iam::633690268896:role/service-role/AmazonRedshift-CommandsAccessRole-20250505T135309'
        ALLOWOVERWRITE 
        FORMAT AS PARQUET;
    """
...
bucket_name = 'redshiftprocessing'
prefix = safe_user +'/sales/parquet'  # Make sure to end with '/' for folder
print("s3 prefix ", prefix)
response = s3.list_objects_v2(
    Bucket=bucket_name,
    Prefix=prefix
...
# MAGIC %md
# MAGIC Creating USERS and SALES delta tables for this lab
...
full_table_name_parquet = f"{uc_schema_name}.{table_name}"
print(full_table_name_parquet)
from pyspark.sql.types import StructType, StructField, IntegerType, StringType, ShortType, DecimalType, TimestampType
...
# Save it as a Delta table if it doesn't exist
   if not spark.catalog.tableExists(full_table_name_parquet):
    ...
    print(f"Error creating sales_parquet_worflow: {e}")
...
sql_query = f"""
COPY INTO {full_table_name}
FROM 's3://redshiftprocessing/{safe_user}/sales/parquet/'
FILEFORMAT = PARQUET
COPY_OPTIONS ('mergeSchema' = 'True')
"""
...
# Construct the SQL query using f-string
sql_query = f"""
select salesid, saletime from  {full_table_name}
"""
...
# MAGIC %md
# MAGIC ## RedShift Create Function
# MAGIC ![Distribution](./images/getminutes.png)

# COMMAND ----------

# MAGIC %md
# MAGIC Databricks Syntax

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE OR REPLACE FUNCTION get_minutes_past_midnight(ts TIMESTAMP)
# MAGIC RETURNS INT
# MAGIC RETURN HOUR(ts) * 60 + MINUTE(ts);

# COMMAND ----------

# MAGIC %md
# MAGIC ## RedShift Create Stored Procedure
# MAGIC ![Distribution](./images/getusersfromco.png)

# COMMAND ----------

# MAGIC %md
# MAGIC Databricks Sytnax CREATE FUNCTION
...
# Construct the SQL query using f-string
sql_query = f"""
CREATE OR REPLACE FUNCTION get_users_from_co_who_like_sports()

  -- Create or replace a temporary view (temp tables not supported in this context)
RETURNS TABLE (first_name STRING, email STRING, total_minutes INT)
RETURN 
SELECT 
      firstname,
      email,
      get_minutes_past_midnight(current_timestamp()) AS total_minutes
  FROM {full_table_name}
  WHERE state = 'CO' AND likesports = TRUE GROUP BY firstname, email
  ORDER BY firstname;
"""
...
# MAGIC %sql
# MAGIC SELECT * FROM get_users_from_co_who_like_sports()
...
# MAGIC %md
# MAGIC Create as SQL table function
# MAGIC
# MAGIC https://docs.databricks.com/aws/en/sql/language-manual/sql-ref-syntax-ddl-create-sql-function#create-a-sql-table-function
...
# Construct the SQL query using f-string
sql_query = f"""
CREATE OR REPLACE FUNCTION f_sql_less(a FLOAT, b FLOAT)
RETURNS FLOAT
...
# Construct the SQL query using f-string
sql_query = f"""
select f_sql_less (commission, pricepaid*0.20) as result from {full_table_name};;
"""
...
# MAGIC %md
# MAGIC ## RedShift Create Stored Procedure
# MAGIC ![Distribution](./images/createpygreater.png)

# COMMAND ----------

# MAGIC %md
# MAGIC Create as user-defined function
# MAGIC
# MAGIC https://docs.databricks.com/aws/en/udf/
...
@udf(FloatType())
def f_py_greater(a: float, b: float) -> float:
    return a if a > b else b

spark.udf.register("f_py_greater", f_py_greater)

# Set up table name
table_name = "sales_parquet_LAB1"
full_table_name = f"{uc_schema_name}.{table_name}"

# Use correct SQL aliasing
sql_query = f"""
SELECT commission AS a, (pricepaid * 0.20) AS b FROM {full_table_name}
"""
df = spark.sql(sql_query)

# Apply the UDF
df = df.withColumn("a", col("a").cast("float"))
df = df.withColumn("b", col("b").cast("float"))
df = df.withColumn("max_val", f_py_greater(col("a"), col("b")))

# Show the result
#df.show()
result_df = df.select("max_val").toPandas()
result_df.head(10)
...
# MAGIC %sql
# MAGIC DESCRIBE FUNCTION EXTENDED f_py_greater

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE FUNCTION EXTENDED f_sql_less
